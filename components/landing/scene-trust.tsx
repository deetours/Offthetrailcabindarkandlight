      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {trustPoints.map((point, index) => (
            <TrustPointCard key={point.num} point={point} index={index} progress={scrollYProgress} />
          ))}
        </div>
      </div>
    </section>
...
function TrustPointCard({
  ...
  const startIn = 0.1 + index * 0.1
  const endIn = startIn + 0.15
  const pointOpacity = useTransform(progress, [startIn, endIn, 0.85, 0.98], [0, 1, 1, 0])
  const pointY = useTransform(progress, [startIn, endIn], [30, 0])

  return (
    <motion.div
      style={{ opacity: pointOpacity, y: pointY }}
    ...
    >
      ...
      <p className="font-sans text-muted-foreground/60 text-sm leading-relaxed">{point.description}</p>
    </motion.div>
  )
}
